...
export default class ProdutoDAO {
    ...
    async init() {
        try 
        {
            ...
            CREATE TABLE IF NOT EXISTS produto(
                ...
                CONSTRAINT fk_categoria FOREIGN KEY(fk_codigo_cat) REFERENCES categoria(prod_codigo)
            );
        `;
    ...
    async alterar(produto) {
        if (produto instanceof Produto) {
            const conexao = await conectar();
            const sql = `UPDATE produto SET prod_descricao=?,prod_precoCusto=?,prod_precoVenda=?,prod_qtdEstoque=?,prod_urlImagem=?,prod_dataValidade=str_to_date(?,'%d/%m/%Y'), fk_codigo_categoria = ?
                WHERE codigo = ?
            `;
            let parametros = [
                produto.descricao,
    ...
    async consultar(termo) {
        ...
        if (isNaN(parseInt(termo))) {
            sql = "SELECT * FROM produto p INNER JOIN categoria c ON p.fk_codigo_cat = c.codigo WHERE descricao LIKE ?";
            parametros = ['%' + termo + '%'];
        }
        else {
            sql = "SELECT * FROM produto p INNER JOIN categoria c ON p.fk_codigo_cat = c.codigo WHERE descricao LIKE ?";
